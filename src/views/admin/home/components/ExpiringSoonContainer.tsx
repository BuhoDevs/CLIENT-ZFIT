...
import { useGetExpiringSoon } from "../../../../hooks/subscriptions";
import { IExpiringSoonDatatable } from "../../../../types/suscription";

moment.locale("es");
const ExpiringSoonContainer = () => {
  ...
  const { data: expiringSoonData, isPending: areExpiringSoonFetching } =
    useGetExpiringSoon({
  ...
  const attendancesColumns: ColumnDef<IExpiringSoonDatatable>[] = [
    {
      header: "FOTO",
      accessorKey: "photo",
      cell: ({ row }) => (
    ...
    },
    {
      ...
      accessorFn: (row) => moment(row.dateIn).locale("es").format("DD/MM/YYYY"),
      accessorKey: "dateIn",
    },

    {
      header: ({ column }) => (
        <CustomHeaderColumn column={column} columnText="FECHA FINAL" />
      ),
      accessorFn: (row) =>
        moment(row.dateOut).locale("es").format("DD/MM/YYYY"),
      accessorKey: "dateOut",
    },
  ];
    ...
    <Box p={1} h="full" bg={bgFilters} borderRadius={8}>
      ...
      >
        <Text
          ...
          lineHeight="100%"
        >
          Suscripciones prontas a expirar
        </Text>
      </Flex>
      <DataTable
        columns={attendancesColumns}
        isFetching={areExpiringSoonFetching}
        isLoading={false}
        tableVariant="simple"
        data={expiringSoonData?.membershipsExpiringSoon}
        // setSelectedItem={setSubscriptionSelected}
      />
      {expiringSoonData && expiringSoonData.totalLength > 0 && (
        <Box
          p="2"
          bg="brand.50"
        ...
        >
          <Pagination
            ...
            totalItems={expiringSoonData?.totalLength}
          >
            <PaginationButtonFirstPage />
            <PaginationButtonPrevPage />
            <PaginationInfo />
